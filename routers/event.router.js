...
const { getEventByCode, getAllEvents } = require("../controllers/event.controller");
...
// GET Event document matching param eventCode
eventRouter.get("/:eventCode", async (req, res) => {
	try {
		let event = await getEventByCode(req.params.eventCode);
		res.status(200).send(event);
	} catch (err) {
